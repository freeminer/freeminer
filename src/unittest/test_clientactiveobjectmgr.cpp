...
{
	client::ActiveObjectMgr caomgr;
	auto tcao_u = std::make_shared<TestClientActiveObject>();
	auto tcao = tcao_u.get();
	...
	tcao_u = std::make_shared<TestClientActiveObject>();
	tcao = tcao_u.get();
	UASSERT(caomgr.registerObject(std::move(tcao_u)));
...
{
	client::ActiveObjectMgr caomgr;
	auto obj_u = std::make_shared<TestSelectableClientActiveObject>(
