...
{
	(void)p;
	// Earth elevation is height above the geoid, not an ocean mask.  A negative
	// elevation can be perfectly dry (polders, salt basins, the Dead Sea shore,
	// and other endorheic depressions).  Arnis places water later from ESA/OSM
	// land cover, so the generic mapgen sea-level rule must not synthesize water.
	return false;
}
		...
		for (pos_t x = node_min.X; x <= node_max.X; x++, index++) {
			...
			for (pos_t y = node_min.Y; y <= node_max.Y; y++) {
				...
				if (underground) {
					...
					// Do not infer oceans from elevation.  Confirmed ESA/OSM water is
					// generated by the Arnis ground/water passes after base terrain.
					vm->m_data[i] = n_air;
				}
