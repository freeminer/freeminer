...
void ServerMap::cancelBlockMake(BlockMakeData *data)
{
	...
	const v3bpos_t full_bpmin = data->blockpos_min - EMERGE_EXTRA_BORDER;
	const v3bpos_t full_bpmax = data->blockpos_max + EMERGE_EXTRA_BORDER;
	for (auto x = full_bpmin.X; x <= full_bpmax.X; x++)
	for (auto z = full_bpmin.Z; z <= full_bpmax.Z; z++)
	for (auto y = full_bpmin.Y; y <= full_bpmax.Y; y++) {
...
MapBlock *ServerMap::loadBlock(const std::string &blob, v3bpos_t p3d, bool save_after_load)
{
...
void ServerMap::transformLiquidsLocal(std::map<v3bpos_t, MapBlock*> &modified_blocks, UniqueQueue<v3pos_t> &liquid_queue,
...
void ServerMap::transformLiquids(std::map<v3pos_t, MapBlock*> &modified_blocks,
