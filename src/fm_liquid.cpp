...
NEXT_LIQUID:;
	while (transforming_liquid_size() > 0) {
		...
		*/
		...
		for (uint8_t e = D_BOTTOM; e <= D_TOP; e++) {
			...
			case LIQUID_NONE:
				...
				// { // need lua drop api for drop torches
						 ...
						 nb.content == melt_kind_flowing && nb.type != NEIGHBOR_UPPER &&
						 !(loopcount % 2)) {
					...
					liquid_levels[i] =
							((float)my_max_level /
									(melt_max_level ? melt_max_level : my_max_level)) *
			...
			infostream << "get node i=" << (int)i << " " << PP(nb.pos)
					   ...
					   << " p1=" << (int)nb.node.param1 << " p2=" << (int)nb.node.param2
					   << " lt="
					   << f.liquid_type
					   ...
					   << " Lmax=" << (int)nodemgr->get(liquid_kind_flowing).getMaxLevel()
					   << std::endl;
#endif
		}
		...
		int16_t pressure =
				liquid_pressure
						? ((ItemGroupList)nodemgr->get(liquid_kind).groups)["pressure"]
						: 0;
		...
		*/
		...
		if (total_level > 0 && neighbors[D_TOP].liquid &&
			...
			int16_t add = (total_level > level_max_compressed - liquid_levels_want[D_TOP])
		...
		for (int8_t r = D_BOTTOM; r <= D_TOP; ++r) {
			...
			*/
			...
			if (liquid_levels_want[i] != liquid_levels[i] && liquid_levels[D_TOP] <= 0 &&
		...
		if (total_was > flowed) {
			...
			for (uint8_t rr = 0; rr <= 6; rr++) {
				infostream << "  i=" << (int)rr << ",b" << (int)liquid_levels[rr] << ",a"
						   << (int)liquid_levels_want[rr];
			}
			infostream << std::endl;
		}
#endif
