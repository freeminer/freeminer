	...
	*/
	{
...

		{
			...
			for (const auto &client : clients) {
				...
				if (client.second->getState() < CS_DefinitionsSent)
					continue;

				// This can happen if the client times out somehow
				if (!m_env->getPlayer(client.second->peer_id))
					continue;

				PlayerSAO *playersao = getPlayerSAO(client.second->peer_id);
				if (!playersao)
					continue;

				// TODO: only for minetest clients
#if MINETEST_PROTO
                if (client.second->getState() < CS_Active || client.second->uptime() < 2)
                    continue;
#endif

				SendActiveObjectRemoveAdd(client.second.get(), playersao);
			}
		}
...
u16 Server::Receive(float min_time)
{
	u16 received {};
	...
	for (;;) {
		...
		try {
			...
			verbosestream << "Server:Receive(): msgpack:" << e.what() << std::endl;
		} catch (const std::exception &e) {
#if !MINETEST_PROTO
			infostream << "Server:Receive(): exception:" << e.what() << std::endl;
#endif
		}
	}
	return received;
...
{
	verbosestream << "Server::deletingPeer(): id=" << peer_id
		<< ", timeout=" << timeout << std::endl;

	m_clients.event(peer_id, CSE_Disconnect);
	DeleteClient(peer_id, timeout ? CDR_TIMEOUT : CDR_LEAVE);
}
...
#if !MINETEST_PROTO
		auto data = obj->getClientInitializationData(client->net_proto_version);
		if (!data.size())
			continue;
...

	{
		...
		for (const session_t client_id : clients) {
			...
			if (client->net_proto_version_fm) {
				total += client->GetNextBlocksFm(m_env, m_emerge.get(), dtime, queue,
						m_uptime_counter->get() + m_env->m_game_time_start, max_ms);
			} else {
				total += client->GetNextBlocks(m_env, m_emerge.get(), dtime, queue, max_ms);
			}
...
bool Server::dynamicAddMedia(const DynamicMediaArgs &a)
{
	...
	if (m_env) {
		...
		for (auto &pair : m_clients.getClientList()) {
			...
			const auto proto_ver = pair.second->net_proto_version.load(std::memory_order::relaxed);
			if (proto_ver < 39)
				continue;
	...
	for (session_t peer_id : delivered) {
		if (auto player = m_env->getPlayer(peer_id))
			getScriptIface()->on_dynamic_media_added(a.token, player->getName().c_str());
	}
...
PlayerSAO* Server::emergePlayer(const char *name, session_t peer_id, u16 proto_version)
{
	...
	*/
	...
	if (!player && maintenance_status) {
		infostream
				<< "emergePlayer(): Maintenance in progress, disallowing loading player"
				<< "\n";
		return {};
	}
	...
	for(;;) {
	   ...
	   // TODO: more errors here
		...
		/*
			Profiler
		*/
       if(server.m_clients.getClientList().size())
...
*/
	...
	if (server.m_autoexit || g_profiler_enabled || profiler_print_interval > 0) {
		infostream << std::fixed << std::setprecision(9) << "\n";
		infostream << "Profiler:" << std::endl;
		g_profiler->print(infostream);
		g_profiler->clear();
	}
