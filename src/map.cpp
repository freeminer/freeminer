	...
	float end_offset, u32 needed_count, bool dense)
{
	...
	const auto pos_origin_f = intToFloat(pos_camera, BS);
	u32 count = 0;
	...
	for (; offset < (dense ? DENSE_CHECK_DISTANCE : distance); offset += step) {
		const auto pos_node_f = pos_origin_f + direction * offset;
...
bool Map::isBlockOccluded(v3pos_t pos_relative, v3pos_t cam_pos_nodes, bool dense)
{
...
std::map<v3bpos_t, bool> MMVManip::getCoveredBlocks() const
{
	...
	const auto &check_block = [this] (v3bpos_t bp) -> bool {
		v3bpos_t pmin = bp * MAP_BLOCKSIZE;
		v3bpos_t pmax = pmin + v3bpos_t(MAP_BLOCKSIZE-1);
		for(bpos_t z=pmin.Z; z<=pmax.Z; z++)
		for(bpos_t y=pmin.Y; y<=pmax.Y; y++)
		for(bpos_t x=pmin.X; x<=pmax.X; x++) {
