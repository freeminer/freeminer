...
struct NearbyCollisionInfo {
	// node
	NearbyCollisionInfo(bool is_ul, int bouncy, v3pos_t pos, const aabb3f &box) :
		obj(nullptr),
		box(box),
		position(pos),
		bouncy(bouncy),
		...
		bool collide_with_objects)
{
	...
	std::vector<NearbyCollisionInfo> cinfo;
	{
		auto min = floatToInt(pos_f + box_0.MinEdge, BS) - v3pos_t(1, 1, 1);
		auto max = floatToInt(pos_f + box_0.MaxEdge, BS) + v3pos_t(1, 1, 1);
