...
void NodeBox::msgpack_pack(msgpack::packer<msgpack::sbuffer> &pk) const
{
	...
	if (type == NODEBOX_WALLMOUNTED) {
		...
		PACK(NODEBOX_S_CONNECTED_TOP, c.connect_top);       // 2
		PACK(NODEBOX_S_CONNECTED_BOTTOM, c.connect_bottom); // 3
		PACK(NODEBOX_S_CONNECTED_FRONT, c.connect_front);   // 4
		PACK(NODEBOX_S_CONNECTED_LEFT, c.connect_left);     // 5
		PACK(NODEBOX_S_CONNECTED_BACK, c.connect_back);     // 6
		PACK(NODEBOX_S_CONNECTED_RIGHT, c.connect_right);   // 7
...
void NodeBox::msgpack_unpack(msgpack::object o)
{
	...
	if (type == NODEBOX_WALLMOUNTED) {
		...
		if (packet.count(NODEBOX_S_CONNECTED_TOP) && packet.count(NODEBOX_S_CONNECTED_RIGHT)) { //lite check
			...
			packet[NODEBOX_S_CONNECTED_TOP].convert(c.connect_top);       // 2
			packet[NODEBOX_S_CONNECTED_BOTTOM].convert(c.connect_bottom); // 3
			packet[NODEBOX_S_CONNECTED_FRONT].convert(c.connect_front);   // 4
			packet[NODEBOX_S_CONNECTED_LEFT].convert(c.connect_left);     // 5
			packet[NODEBOX_S_CONNECTED_BACK].convert(c.connect_back);     // 6
			packet[NODEBOX_S_CONNECTED_RIGHT].convert(c.connect_right);   // 7
		}
	}
...
//#if CHECK_CLIENT_BUILD()
//		f.visuals->minimap_color = video::SColor(0,0,0,0);
//#endif
		// Insert directly into containers
...
//#endif
		...
		m_content_lighting_flag_cache[c] = f.getLightingFlags();
		addNameIdMapping(c, f.name);
		// mtproto: 0 must be ignored always
		if (c)
		m_content_features[c] = std::move(f);
	}
}
...
void NodeDefManager::msgpack_unpack(msgpack::object o)
{
	...
	ContentFeatures new_f;
	for (auto it = unpacked_features.begin();
