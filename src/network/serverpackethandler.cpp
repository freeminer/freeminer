...
void Server::handleCommand_Init(NetworkPacket* pkt)
{
	...
	*/
	...
	if (!g_settings->getBool("enable_any_name") && !string_allowed(playerName, PLAYERNAME_ALLOWED_CHARS)) {
	...
	case INTERACT_USE: {
		...
		if (m_script->item_OnUse(selected_item, playersao, pointed)) {
            ...
            if (selected_item.has_value())
				stat.add("use_" + selected_item->name, player->getName());
		}
...
void Server::handleCommand_NodeMetaFields(NetworkPacket* pkt)
{
	...
	*pkt >> p >> formname;
	...
	static const thread_local auto enable_rollback_recording  = g_settings->getBool("enable_rollback_recording");
	if (!enable_rollback_recording) {
