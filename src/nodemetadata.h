...
public:
	...
	// Return all keys
	std::vector<v3pos_t> getAllKeys() const;
	// Get pointer to data
	NodeMetadata *get(v3pos_t p) const;

	/// @brief Deletes data
	/// @return the old data (if it was owned)
	std::unique_ptr<NodeMetadata> remove(v3pos_t p);
	/// @brief Deletes old data and sets a new one
	/// @return the old data (if it was owned)
	std::unique_ptr<NodeMetadata> set(v3pos_t p, NodeMetadata *d);
