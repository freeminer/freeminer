	...
	m_modchannel_mgr(new ModChannelMgr())
{
	...
	// Register client-specific settings callbacks
	registerClientSettingsCallbacks();
    // ===
	...
	*/
	{
		...
		if(counter <= 0.0f)
		{
			...
			verbosestream << "Client packetcounter (" << counter << "s): "
...
*/
	...
	*/
	{
		for (auto &m_sounds_to_object : m_sounds_to_objects) {
			...
			if (!cao)
				continue;
			m_sound->updateSoundPosVel(client_id, oposToV3f(cao->getPosition() * (1.0f/BS)),
	...
	*/
	...
	static thread_local const auto timelapse = g_settings->getFloat("timelapse");
	if (timelapse) {
...
void Client::peerAdded(session_t peer_id)
{
	infostream << "Client::peerAdded(): peer->id="
			<< peer_id << std::endl;
}

void Client::deletingPeer(session_t peer_id, bool timeout)
{
...
void writePlayerPos(LocalPlayer *myplayer, ClientMap *clientMap, NetworkPacket *pkt, bool camera_inverted)
{
	...
	u8 wanted_range  = std::fmin(255.0f,
			std::ceil(clientMap->getServerWantedRange() * (1.0f / MAP_BLOCKSIZE)));
...
{
	m_out_chat_queue = decltype(m_out_chat_queue) ();
}
...
void Client::sendPlayerPos()
{
	...
	u8 wanted_range = std::fmin(255.0f,
			std::ceil(map.getServerWantedRange() * (1.0f / MAP_BLOCKSIZE)));
...
bool Client::getChatMessage(std::wstring &res)
{
	...
	ChatMessage *chatMessage = m_chat_queue.pop_front();
	//m_chat_queue.pop();
...
{
	static auto thread_local headless_optimize = g_settings->getBool("headless_optimize");
