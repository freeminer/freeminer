...
Json::Value MCPPlayerControl::sendMCPRequest(const std::string& method, const Json::Value& params) {
    ...
    if (m_mcp_stdin >= 0) {
        // Send via stdio
        ssize_t bytes_written = write(m_mcp_stdin, request_str.c_str(), request_str.length());
        if (bytes_written == -1) {
        ...
        if (select_result > 0 && FD_ISSET(m_mcp_stdout, &read_fds)) {
            ...
            if (bytes_read > 0) {
                ...
                if (jsonReader->parse(response_str.c_str(), response_str.c_str() + response_str.length(), &response, &errors)) {
                    return response;
                } else {
...
void MCPPlayerControl::updatePlayerState() {
    ...
    if (response.isMember("result") && response["result"].isMember("content")) {
        ...
        std::unique_ptr<Json::CharReader> jsonReader(reader.newCharReader());
        if (jsonReader->parse(state_json.c_str(), state_json.c_str() + state_json.length(), &player_state, &errors)) {
            // Update local player state
            LocalPlayer* player = m_client->getEnv().getLocalPlayer();
            if (player) {
                v3f new_pos(
                    player_state["position"]["x"].asFloat() * BS,
...
void MCPPlayerControl::movePlayerTo(float x, float y, float z) {
    LocalPlayer* player = m_client->getEnv().getLocalPlayer();
    if (player) {
        player->setPosition(v3f(x * BS, y * BS, z * BS));
