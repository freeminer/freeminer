...
public:
	...
	void onSetUniforms(video::IMaterialRendererServices *services) override
	{
...

       {
		...
		//FMTODO!!
		{
			const auto *inventory = &m_client->getEnv().getLocalPlayer()->inventory;
			ItemStack playeritem;
			{
				auto *mlist = inventory->getList("main");
				if (mlist != NULL) {
					playeritem = mlist->getItem(
		...
		auto offset = posToFloat(m_client->getCamera()->getOffset(), BS);
...
//	try {
		...
		showOverlayMessage(std::string("Oops ") , 0, 0);
		return false;
#else
	} catch (int) { //nothing
		return false;
...
void Game::updateCameraOffset()
{
	...
	auto old_camera_offset = camera->getOffset();

	camera->updateOffset();

	auto camera_offset = camera->getOffset();
...
void Game::processPlayerInteraction(f32 dtime, bool show_hud)
{
	...
	*/
	...
	if (isTouchShootlineUsed()) {
		...
		shootline.start += intToFloat(camera_offset, (opos_t)BS);
		shootline.end += intToFloat(camera_offset, (opos_t)BS);
	}
	...
	if (place_param2) {
		...
		if (abs(dir.Y) > MYMAX(abs(dir.X), abs(dir.Z))) {
			...
			if (selected_def.wallmounted_rotate_vertical) {
				...
				auto pdir = v3opos_t::from(neighborpos) - ppos;
...
void Game::showOverlayMessage(const std::string &msg, float dtime, int percent, float *indef_pos)
{
