	...
	const v3f &sneak_max)
{
	...
	const v3pos_t current_node = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	v3pos_t pp2 = floatToInt(position + v3f(0.0f, -0.2f * BS, 0.0f), BS);
	node = map->getNode(pp, &is_valid_position);
	...
	std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	v3pos_t current_node = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
	if (m_sneak_node_exists &&
	...
	if (m_need_to_get_new_sneak_node && physics_override_sneak) {
		...
		v3pos_t pos_i_bottom = floatToInt(position - v3f(0.0f, position_y_mod, 0.0f), BS);
