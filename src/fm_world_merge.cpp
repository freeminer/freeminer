...
void *WorldMergeThread::run()
{
	...
	g_settings->getU64NoEx("world_merge_throttle", abm_world_throttle);
	u64 abm_world_max_clients = m_server->isSingleplayer() ? 1 : 0;
	g_settings->getU64NoEx("world_merge_max_clients", abm_world_max_clients);
...
// Yes, very bad
		...
		infostream << "Abm world run " << run << " blocks " << loadable_blocks_size
				   ...
				   << world_merge_last << " max_clients " << abm_world_max_clients
