...
confdir=`pwd`
run_opts="--config $confdir/freeminer.headless.conf --worldname autotest --go --autoexit 100"
logdir=`pwd`/logs
...
./freeminer $run_opts --logfile $logdir/autotest.$name.game.log >> $logdir/autotest.$name.out.log 2>>$logdir/autotest.$name.err.log
cd ..
...
time nice make -j $(nproc || sysctl -n hw.ncpu || echo 2)
valgrind ./freeminer $run_opts --logfile $logdir/autotest.$name.game.log >> $logdir/autotest.$name.out.log 2>>$logdir/autotest.$name.err.log
cd ..
