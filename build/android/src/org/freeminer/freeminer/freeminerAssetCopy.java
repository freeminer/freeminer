...
public class freeminerAssetCopy extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		freeminerAssetCopy prevActivity = 
				(freeminerAssetCopy) getLastNonConfigurationInstance();
	...
	private class copyAssetTask extends AsyncTask<String, Integer, String>
	{
		...
		protected String doInBackground(String... files)
		{
			...
			if (!TempFolder.exists())
			{
				...
				{
					Log.v("freeminerAssetCopy","deleting: " + todel[i].getAbsolutePath());
					todel[i].delete();
				}
			}
			...
			try {
				...
				Log.e("freeminerAssetCopy","Failed to create .nomedia file");
				e.printStackTrace();
			}
			...
			for (int i = 0; i < m_tocopy.size(); i++)
			{
				try
				{
					...
					try
					{
						...
						Log.e("freeminerAssetCopy","Copying file: " + filename + " FAILED (not in assets)");
						e.printStackTrace();
						continue;
					}
					...
					if (len > 0)
					{
						...
						try
						{
							...
							Log.e("freeminerAssetCopy","Copying file: " + baseDir +
						...
						dst.close();
						Log.v("freeminerAssetCopy","Copied file: " +
		...
		protected void onProgressUpdate(Integer... progress)
		{
			...
			if (m_copy_started)
			{
				...
				while (bounds.width() > getResources().getDisplayMetrics().widthPixels * 0.7) {
					Log.e("freeminerAssetCopy", todisplay + ": " +
			...
			while (itr.hasNext())
			{
					...
					if (!current_folder.exists())
					 {
						...
						{
							Log.e("freeminerAssetCopy","\t failed create folder: " +
									FlashPath);
						}
						else
						{
							Log.v("freeminerAssetCopy","\t created folder: " +
									FlashPath);
						}
					}
				...
				if (testme.exists())
				{
					...
					catch (IOException e)
					{
						...
						Log.e("freeminerAssetCopy","Failed to open asset file \"" +
		...
		protected void BuildFolderList()
		{
			...
			{
				Log.e("freeminerAssetCopy","Error on processing index.txt");
				e1.printStackTrace();
			}
		}
		...
		protected void BuildFileList()
		{
			...
			{
				Log.e("freeminerAssetCopy","Error on processing filelist.txt");
				e1.printStackTrace();
			}
		}
